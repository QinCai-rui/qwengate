 ...
 * Returns the cookie jar (acw_tc, cdn_sec_tc, ssxmod_itna, token, etc.)
 * or just the JWT as a fallback.
 */
export function getGlmCookies(jwt: string): string {
  const accounts = getAccounts();
  for (const acct of accounts) {
    const state = getProviderState(acct.email, 'glm');
    if (state?.token === jwt && state.cookies) {
      return state.cookies;
    }
  }
  // Fallback: just send the token
  return `token=${jwt}`;
}
...
export async function getCurrentUser(jwt: string): Promise<GlmUser | null> {
  try {
    ...
    const cookieStr = getGlmCookies(jwt);
  ...
  try {
    ...
    try {
      const sessionsRes = await wreqFetch(`${GLM_BASE_URL}/api/v1/chats/`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${jwt}`, cookie: cookieStr },
        timeout: 10,
        impersonate: 'chrome_142',
      });
