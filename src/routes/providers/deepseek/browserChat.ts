...
async function page(targetId: string) {
  ...
  let id = 0; const p = new Map<number,any>();
  return new Promise<any>((resolve, reject) => {
    ...
    ws.onopen = () => { clearTimeout(t); opened = true; };
    ws.onerror = () => { clearTimeout(t); reject(new Error('CDP page connection error')); };
    ws.onmessage = e => {
      const m = JSON.parse(e.data as string);
      if (m.id !== undefined && p.has(m.id)) {
        const h = p.get(m.id)!; p.delete(m.id);
    ...
    const poll = setInterval(() => {
      if (ws.readyState === WebSocket.OPEN) {
        ...
        opened = true;
        resolve({
          send: (method: string, params?: any): Promise<any> => new Promise((rs, rj) => {
            const i = ++id; p.set(i, { rs, rj, _m: method });
            ws.send(JSON.stringify({ id: i, method, params }));
            setTimeout(() => { if (p.has(i)) { p.delete(i); rj(new Error('cdp:' + method)); } }, 60000);
          }),
...
async function findOrCreatePage(): Promise<any> {
  try {
    ...
    let bid = 0; const mp = new Map<number, any>();
    await new Promise((r) => {
      const t = setTimeout(() => { try { bw.close(); } catch {}; r(undefined); }, 5000);
      bw.onopen = () => { clearTimeout(t); r(undefined); };
      bw.onerror = () => { clearTimeout(t); r(undefined); };
    });
    bw.onmessage = e => {
      const m = JSON.parse(e.data as string);
      if (m.id !== undefined && mp.has(m.id)) {
        const h = mp.get(m.id)!; mp.delete(m.id);
        m.error ? h.rj(new Error(JSON.stringify(m.error))) : h.rs(m.result);
      }
    };
    const ct: any = await new Promise((rs, rj) => {
      const i = ++bid; mp.set(i, { rs, rj });
...
export async function chatViaCdpBrowser(c: any, body: any, _email: string): Promise<Response> {
  ...
  if (!p) {
    return c.json({ error: { message: 'No CDP browser available. Start Chrome with --remote-debugging-port=9222.', type: 'browser_error' } }, 503);
  }
  ...
  try {
    ...
    if (textareaRes.result?.value === 'NOT FOUND') {
      return c.json({ error: { message: 'DeepSeek page textarea not found — page may not have loaded properly.', type: 'browser_error' } }, 502);
    }
