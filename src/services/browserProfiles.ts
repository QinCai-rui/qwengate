...
export async function extractProviderToken(
...
): Promise<{ token: string; expiresAt: number; wafToken?: string; captchaVerifyParam?: string; cookieStr?: string } | null> {
  ...
  if (provider === 'glm') {
    // GLM: read JWT from token cookie + ALL cookies for full cookie jar
    try {
      const cookies = await context.cookies();
      const tokenCookie = cookies.find((c: any) => c.name === 'token' && (c.domain.includes('z.ai') || c.domain.includes('chatglm')));
      if (tokenCookie?.value) {
        // Build full cookie string from ALL cookies (acw_tc, cdn_sec_tc, ssxmod_itna, etc.)
        const cookieStr = cookies
          .filter((c: any) => c.value && (c.domain.includes('z.ai') || c.domain.includes('chatglm') || c.domain.includes('aliyuncs')))
          .map((c: any) => `${c.name}=${c.value}`)
          .join('; ');
      ...
      try {
        ...
        if (!opts.authPagePaths.some((p: string) => currentUrl.includes(p))) {
          ...
          if (tokenResult) {
            ...
            return {
              status: 'success',
              cookieStr: tokenResult.cookieStr || '',
