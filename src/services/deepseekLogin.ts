...
async function extractDeepSeekToken(page: any): Promise<string | null> {
  try {
    ...
    const tokenJson: string | null = await page.evaluate(function () {
      try {
    ...
    try {
      const tokenFromApi: string | null = await page.evaluate(async function () {
        try {
...
export async function loginDeepseekAuto(
  ...
  const outcome: AutoLoginOutcome = await autoLoginViaBrowser(email, password, {
    provider: 'deepseek',
...
export async function loginDeepSeekManual(email: string, password: string): Promise<ProviderAuthState | null> {
  const state = await manualBrowserLogin(email, password, {
  ...
  if (state && state.token) {
    try {
      ...
      const context = await setupBrowserContext(email, false);
      const page = context.pages()[0] || (await context.newPage());

      try {
        await page.goto(DEEPSEEK_CHAT_URL, { waitUntil: 'networkidle', timeout: 15000 });
        const token = await extractDeepSeekToken(page);
        if (token) {
